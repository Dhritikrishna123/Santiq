...
class PluginConfig(BaseModel):
    ...
    on_error: str = Field(default="stop", pattern="^(stop|continue|retry)$", description="Error handling strategy")
    enabled: bool = Field(default=True, description="Whether the plugin is enabled")
    timeout: Optional[int] = Field(default=None, gt=0, description="Plugin timeout in seconds")
...
class PipelineConfig(BaseModel):
    ...
    profilers: List[PluginConfig] = Field(default_factory=list, description="Data profiling plugins")
    transformers: List[PluginConfig] = Field(default_factory=list, description="Data transformation plugins")
    loaders: List[PluginConfig] = Field(..., description="Data loading plugins")
    
    # Global settings
    cache_intermediate_results: bool = Field(default=True, description="Whether to cache intermediate results")
    max_memory_mb: Optional[int] = Field(default=None, gt=0, description="Maximum memory usage in MB")
    temp_dir: Optional[str] = Field(default=None, description="Temporary directory path")
    parallel_execution: bool = Field(default=False, description="Enable parallel execution where possible")
    ...
    def validate_temp_dir(cls, v: Optional[str]) -> Optional[str]:
        if v is not None:
            ...
            if not temp_path.exists():
                ...
                raise ValueError(f'Temporary directory path {v} exists but is not a directory')
        return v
...
class ConfigManager:
    ...
    def load_pipeline_config(self, config_path: str) -> PipelineConfig:
        ...
        if not config_file.is_file():
            raise PipelineConfigError(f"Configuration path is not a file: {config_path}")
        ...
        try:
            ...
            raise PipelineConfigError(f"Error parsing YAML configuration file {config_path}: {e}")
        except FileNotFoundError:
            raise PipelineConfigError(f"Configuration file not found: {config_path}")
        except PermissionError:
            raise PipelineConfigError(f"Permission denied reading configuration file: {config_path}")
        except Exception as e:
            raise PipelineConfigError(f"Unexpected error loading configuration file {config_path}: {e}")
    ...
    def save_preferences(self, preferences: Dict[str, Any], preference_file: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            raise PipelineConfigError(f"Cannot create preference directory {preference_path.parent}: {e}")

        try:
            with open(preference_path, 'w', encoding='utf-8') as f:
                yaml.dump(preferences, f, default_flow_style=False, sort_keys=True, indent=2)
        except (OSError, PermissionError) as e:
            raise PipelineConfigError(f"Permission denied writing preferences to {preference_file}: {e}")
        except Exception as e:
            raise PipelineConfigError(f"Error saving preferences to {preference_file}: {e}")
    ...
    def load_preferences(self, preference_file: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:    
            ...
            raise PipelineConfigError(f"Error parsing preference file {preference_file}: {e}")
        except (OSError, PermissionError) as e:
            raise PipelineConfigError(f"Error reading preference file {preference_file}: {e}")
        except Exception as e:
            raise PipelineConfigError(f"Unexpected error loading preferences from {preference_file}: {e}")
