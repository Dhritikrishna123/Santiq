...
class Pipeline:
    ...
    def _execute_profiling(self, context: PipelineContext) -> List[ProfileResult]:
        ...
        for profiler_config in context.config.profilers:
            ...
            try:
                ...
                self.plugin_manager.cleanup_plugin_instance(profiler_config.plugin, "profiler")
        
        return results
    
    def _execute_transformations(self, context: PipelineContext, mode: str) -> pd.DataFrame:
        """Execute transformation plugins."""
        current_data = context.data.copy() if context.data is not None else pd.DataFrame()
        ...
        for transformer_config in context.config.transformers:
            ...
            try:
                ...
                if mode in ["manual", "half-auto"]:
                    suggestions = transformer.suggest_fixes(current_data, 
                                                          self._get_relevant_issues(context.profile_results))
                    if mode == "manual":
                        # In manual mode, user would review suggestions via CLI/UI
                        approved_suggestions = self._get_user_approval(suggestions)
                    else:  # half-auto
                        approved_suggestions = self._auto_approve_known_fixes(suggestions)
                ...
                self.audit_logger.log_event(
                    ...
                    plugin_type="transformer",
                    data={
                        "rows_before": len(context.data) if context.data is not None else 0,
                ...
                self.plugin_manager.cleanup_plugin_instance(transformer_config.plugin, "transformer")
    ...
    def _execute_loading(self, context: PipelineContext) -> List[Dict[str, Any]]:
        ...
        for loader_config in context.config.loaders:
            ...
            try:
                ...
                self.plugin_manager.cleanup_plugin_instance(loader_config.plugin, "loader")
        
        return results
    
    
    def _get_relevant_issues(self, profile_results: List[ProfileResult]) -> List[Dict[str, Any]]:
    ...
    def _get_user_approval(self, suggestions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _auto_approve_known_fixes(self, suggestions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
