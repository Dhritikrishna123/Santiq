...
class DataManager:
    ...
    def __init__(self, temp_dir: Optional[Path] = None, memory_threshold_mb: int = 100) -> None:
    ...
    def save_temp_data(self, data: pd.DataFrame, name: str) -> Path:
        ...
        temp_file = self.temp_dir / f"{name}_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.parquet"
    ...
    if expected_schema:
        ...
        for column in expected_columns & actual_columns:
            ...
            if not _types_compatible(actual_type, expected_type):
                issues.append(f"Column '{column}' type mismatch: expected {expected_type}, got {actual_type}")
