...
class TestExternalPluginConfiguration:
    ...
    def test_remove_external_plugin_config(self, temp_dir: Path):
        ...
        external_plugins = plugin_manager.list_external_plugins()
        assert (
            "profiler" not in external_plugins or len(external_plugins["profiler"]) == 0
        )
...
class TestExternalPluginDiscovery:
    ...
    def test_discover_plugins_includes_external(self, temp_dir: Path):
        ...
        external_plugin = next(
            (p for p in extractor_plugins if p["name"] == "external_extractor"), None
        )
    ...
    def test_external_plugin_installation_status(self, temp_dir: Path):
        ...
        plugin_manager.add_external_plugin_config(
            "installed_transformer", plugin_config
        )

        # Mock package installation check on the external manager
        with patch.object(
            plugin_manager.external_manager, "_is_package_installed"
        ) as mock_installed:
...
class TestExternalPluginValidation:
    ...
    def test_missing_required_fields(self, temp_dir: Path):
        ...
        incomplete_config = {"type": "extractor", "description": "Incomplete plugin"}

        plugin_manager.add_external_plugin_config(
            "incomplete_plugin", incomplete_config
        )
        ...
        incomplete_plugin = next(
            (
                p
                for p in external_plugins["extractor"]
                if p["name"] == "incomplete_plugin"
            ),
            None,
        )
