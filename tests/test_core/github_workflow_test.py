...
class TestGitHubWorkflowCompatibility:
    ...
    def test_cli_availability(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "santiq.cli.main", "--help"],
            capture_output=True,
            text=True,
        )
    ...
    def test_basic_pipeline_execution_ci(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config = PipelineConfig(
                ...
                loaders=[
                    {"plugin": "csv_loader", "params": {"path": str(output_path)}}
                ],
            )
    ...
    def test_environment_variables_in_config(self):
        ...
        try:
            with tempfile.NamedTemporaryFile(
                ...
                assert (
                    "/test/input/data.csv" in pipeline_config.extractor.params["path"]
                )
                assert (
                    "/test/output/result.csv"
                    in pipeline_config.loaders[0].params["path"]
                )
